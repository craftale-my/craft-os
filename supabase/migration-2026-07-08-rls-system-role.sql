-- ============================================================================
-- RLS: align management policies with system_role (2026-07-08, v2)
-- Root cause: the UI gates on staff.system_role (RBAC) but management RLS
-- still checked staff.rank — users promoted via Settings → System Roles saw
-- every manager button while the DB silently filtered writes to 0 rows.
-- Each policy now accepts its original rank check OR the system_roles whose
-- DEFAULT_SYSTEM_ROLE_CAPS grant that capability (per-capability sets, so hr
-- does NOT gain schedule/missions/reviews/settings access it never had):
--   settings-tier  ('manager','admin','owner')
--   schedule       ('supervisor','manager','admin','owner')
--   reviews        ('supervisor','manager','admin','owner')
--   missions       ('manager','admin','owner')
--   hr/staff ops   ('hr','supervisor','manager','admin','owner')
--   salary         ('hr','manager','admin','owner')
-- Also adds the missing mission_completions DELETE policy (Reset Progress
-- silently no-oped for everyone). Idempotent — safe to re-run.
-- ============================================================================
...
create policy "accrual_log_select" on leave_accrual_log for select to authenticated
  using (staff_id = auth.uid() or (current_rank() in ('supervisor','manager') or current_system_role() in ('hr','supervisor','manager','admin','owner')));
...
create policy "attendance_breaks_manage" on attendance_breaks for all to authenticated
  using ((current_rank() in ('supervisor','manager') or current_system_role() in ('hr','supervisor','manager','admin','owner')));
  ...
  using (
    exists (
      ...
      where a.id = attendance_id
        and (a.staff_id = auth.uid() or (current_rank() in ('supervisor','manager') or current_system_role() in ('hr','supervisor','manager','admin','owner')))
    )
  );
...
create policy "branches_manage" on branches for all
  using ((current_rank() = 'manager' or current_system_role() in ('manager','admin','owner')));
...
create policy "company_assets_manager_write" on storage.objects for all
  using (bucket_id = 'company-assets' and (current_rank() = 'manager' or current_system_role() in ('manager','admin','owner')));
...
create policy "completions_update" on mission_completions for update
  using ((current_rank() in ('supervisor','manager') or current_system_role() in ('supervisor','manager','admin','owner')));
...
create policy "missions_insert" on missions for insert
  with check ((current_rank() = 'manager' or current_system_role() in ('manager','admin','owner')));
...
create policy "probation_insert" on probation_reviews for insert
  with check ((current_rank() in ('supervisor','manager') or current_system_role() in ('supervisor','manager','admin','owner')));
...
create policy "probation_select" on probation_reviews for select
  using (staff_id = auth.uid() or (current_rank() in ('supervisor','manager') or current_system_role() in ('supervisor','manager','admin','owner')));
...
create policy "salary_manage" on salary_records for all to authenticated
  using ((current_rank() = 'manager' or current_system_role() in ('hr','manager','admin','owner')));
...
create policy "salary_select_own" on salary_records for select to authenticated
  using (staff_id = auth.uid() or (current_rank() = 'manager' or current_system_role() in ('hr','manager','admin','owner')));
...
create policy "staff_avatars_update" on storage.objects for update to authenticated
  using (
    ...
    and ((storage.foldername(name))[1] = auth.uid()::text or (current_rank() = 'manager' or current_system_role() in ('hr','supervisor','manager','admin','owner')))
  )
  with check (
    bucket_id = 'staff-avatars'
    and ((storage.foldername(name))[1] = auth.uid()::text or (current_rank() = 'manager' or current_system_role() in ('hr','supervisor','manager','admin','owner')))
...
create policy "staff_insert" on staff for insert
  with check ((current_rank() in ('manager') or current_system_role() in ('hr','supervisor','manager','admin','owner')));

drop policy if exists "staff_update" on staff;
create policy "staff_update" on staff for update
  using (id = auth.uid() or (current_rank() in ('supervisor','manager') or current_system_role() in ('hr','supervisor','manager','admin','owner')));

-- Missing DELETE policy: Reset Progress (StaffProfile) deletes mission_completions.
...
-- Same rank-vs-system_role gap inside the accrual engine guard (Settings →
-- Recalculate). Accepts rank OR an HR-capable system_role; cron (no auth.uid)
-- skips the guard as before.
  ...
  if auth.uid() is not null then
    select coalesce(
      (select rank in ('supervisor','manager')
              or system_role in ('hr','supervisor','manager','admin','owner')
       from staff where id = auth.uid()), false) into v_ok;
    if not v_ok then
