...
-- ============================================================
-- 补充: 代码用到但 schema.sql 漏建的 3 个存储桶 + 策略
-- (attendance-selfies / mission-proofs / task-attachments)
-- ============================================================
insert into storage.buckets (id, name, public) values ('attendance-selfies', 'attendance-selfies', true) on conflict (id) do nothing;
drop policy if exists "attendance_selfies_read" on storage.objects;
create policy "attendance_selfies_read" on storage.objects for select using (bucket_id = 'attendance-selfies');
drop policy if exists "attendance_selfies_write" on storage.objects;
create policy "attendance_selfies_write" on storage.objects for insert to authenticated with check (bucket_id = 'attendance-selfies');

insert into storage.buckets (id, name, public) values ('mission-proofs', 'mission-proofs', true) on conflict (id) do nothing;
drop policy if exists "mission_proofs_read" on storage.objects;
create policy "mission_proofs_read" on storage.objects for select using (bucket_id = 'mission-proofs');
drop policy if exists "mission_proofs_write" on storage.objects;
create policy "mission_proofs_write" on storage.objects for insert to authenticated with check (bucket_id = 'mission-proofs');
...
create policy "task_attachments_read" on storage.objects for select using (bucket_id = 'task-attachments');
drop policy if exists "task_attachments_write" on storage.objects;
create policy "task_attachments_write" on storage.objects for insert to authenticated with check (bucket_id = 'task-attachments');
